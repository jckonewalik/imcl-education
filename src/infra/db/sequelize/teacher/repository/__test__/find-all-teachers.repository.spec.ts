...
const makeSut = async (): Promise<Sut> => {
  const teacher1 = makeTeacher({ name: "Joao", active: false });
  const teacher2 = makeTeacher({ name: "Amanda", gender: Gender.F });
  const teacher3 = makeTeacher({ name: "Leandro" });
